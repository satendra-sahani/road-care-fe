  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1D29]">Plan Pricing</h1>
        ...
        <div className="flex gap-2">
          <button onClick={restoreDefaults}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-[#1A1D29] hover:bg-gray-50">
            <RefreshCw className="h-4 w-4" /> Restore defaults
          </button>
          <button onClick={openNew}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#1B3B6F] text-white text-sm font-semibold hover:bg-[#16305C]">
        ...
        {TABS.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap border ${on ? 'bg-[#1B3B6F] text-white border-[#1B3B6F]' : 'bg-white text-[#1A1D29] border-gray-200 hover:bg-gray-50'}`}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {list.map((p) => {
            ...
            return (
              <div key={p._id} className={`bg-white rounded-2xl border p-5 flex flex-col ${p.isActive ? 'border-gray-200' : 'border-dashed border-gray-300 opacity-60'}`}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <label className="inline-flex items-center cursor-pointer shrink-0" title={p.isActive ? 'Active' : 'Inactive'}>
                ...
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="text-2xl font-extrabold text-[#1B3B6F]">
                ...
                <ul className="mt-3 space-y-1 flex-1">
                  {(p.benefits || []).slice(0, 4).map((b, i) => (
                    <li key={i} className="text-[13px] text-gray-600 flex gap-1.5"><span className="text-[#1BA672]">✓</span><span>{b}</span></li>
                  ))}
                  {(p.benefits?.length || 0) > 4 && <li className="text-xs text-gray-400 font-semibold">+{p.benefits.length - 4} more…</li>}
                </ul>

                {/* Actions */}
                <div className="mt-4 flex gap-2 pt-3 border-t border-gray-100">
                  <button onClick={() => openEdit(p)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-[#1B3B6F] hover:bg-[#EAF0F9]">
                    <Pencil className="h-3.5 w-3.5" /> Edit
                  </button>
                  <button onClick={() => remove(p)}
                    className="px-3 py-2 rounded-xl border border-gray-200 text-red-500 hover:bg-red-50" title="Delete">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center overflow-y-auto p-4 lg:p-8">
          <div className="bg-white rounded-2xl w-full max-w-2xl p-6 my-4">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setEditing(null)} className="p-1.5 rounded-lg hover:bg-gray-100"><X className="h-5 w-5" /></button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {isNew && (
                <>
                  <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">PLAN TYPE
                    <select value={editing.planType} onChange={(e) => setEditing({ ...editing, planType: e.target.value as PlanType })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium text-[#1A1D29]">
                  ...
                  <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">KEY (unique, e.g. bike_plus)
                    <input value={editing.key || ''} onChange={(e) => setEditing({ ...editing, key: e.target.value })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
                  </label>
                </>
              )}
              <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">NAME
                <input value={editing.name || ''} onChange={(e) => setEditing({ ...editing, name: e.target.value })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
              </label>
              <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">VEHICLE
                <select value={editing.vehicleType} onChange={(e) => setEditing({ ...editing, vehicleType: e.target.value as any })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium">
              ...
              <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">EMOJI
                <input value={editing.emoji || ''} onChange={(e) => setEditing({ ...editing, emoji: e.target.value })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
              </label>
              <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">TAG (e.g. BEST VALUE)
                <input value={editing.tag || ''} onChange={(e) => setEditing({ ...editing, tag: e.target.value })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
              </label>
              ...
              {editing.planType === 'gps_hardware' ? (
                <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">CUSTOMER PRICE (ONE-TIME ₹)
                  <input type="number" value={editing.priceOneTime ?? 0} onChange={(e) => setEditing({ ...editing, priceOneTime: Number(e.target.value) })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
                </label>
              ) : (
                <>
                  <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">PRICE / YEAR (₹)
                    <input type="number" value={editing.priceYearly ?? 0} onChange={(e) => setEditing({ ...editing, priceYearly: Number(e.target.value) })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
                  </label>
                  <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">PRICE / MONTH (₹, optional)
                    <input type="number" value={editing.priceMonthly ?? 0} onChange={(e) => setEditing({ ...editing, priceMonthly: Number(e.target.value) })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
                  </label>
                </>
              )}
              <label className="text-xs font-bold text-gray-500 flex flex-col gap-1.5">SORT ORDER
                <input type="number" value={editing.sortOrder ?? 0} onChange={(e) => setEditing({ ...editing, sortOrder: Number(e.target.value) })} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-medium" />
              </label>
            </div>
                  ...
                  {(editing.components || []).map((c, i) => (
                    <div key={i} className="flex gap-2">
                      <input value={c.name} placeholder="Component / service name"
                        ...
                        className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm font-medium" />
                      <div className="relative w-32">
                        ...
                        <input type="number" value={c.cost}
                          ...
                          className="w-full border border-gray-200 rounded-xl pl-7 pr-2 py-2 text-sm font-medium" />
                      </div>
                      <button onClick={() => setEditing({ ...editing, components: (editing.components || []).filter((_, j) => j !== i) })}
                        className="px-2 rounded-xl text-red-400 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  ))}
                </div>
                <div className="mt-2 text-sm font-bold text-[#1A1D29] flex justify-between bg-[#F5F7FA] rounded-xl px-3 py-2.5">
                  <span>Total cost ≈ {inr(componentTotal(editing.components))}</span>
                  <span className={((editing.priceOneTime || 0) - componentTotal(editing.components)) >= 0 ? 'text-[#1BA672]' : 'text-red-500'}>
            ...
            <div className="mt-6 flex gap-3 justify-end">
              <button onClick={() => setEditing(null)} className="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-[#1A1D29] hover:bg-gray-50">Cancel</button>
              <button onClick={saveEdit} disabled={saving}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#1B3B6F] text-white text-sm font-semibold hover:bg-[#16305C] disabled:opacity-60">
